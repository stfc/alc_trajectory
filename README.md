...
* [***CI-tests***](./CI-tests): contains the tests files (in .tar format) needed for CI purposes. The user should execute the available scripts of the [***tools***](./tools) folder to run the tests automatically and verify the code has been installed properly (see the [build_code.md](./build_code.md) file for instructions).  
